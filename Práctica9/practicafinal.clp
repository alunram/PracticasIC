;;;; Unidad de practicas 6, Ingenieria del conocimiento ;;;;;;;
;;;; Alvaro Luna Ramirez, 4º DGIIM ;;;;;
;;;; Sistema experto: Tengo un viaje para usted;;;;;
;;;; Utilizar en nuestro sistema el manejo con la incertidumbre ;;;;;;
...
;;;;; Para la práctica 6 añado logica por defecto:
;;;;;       - Si no quiere lugares con mucha gente, le gusta la naturaleza
;;;;;       - Si le gusta la naturaleza, no quiere lugares con mucha gente
;;;;;       - Si quiere calor, le gustará la playa
;;;;;       - Si le gusta la playa, quiere calor
...
(defrule pregnaturaleza
    ...
    (assert (respuestaprovisional naturaleza (upcase (read))))
)
...
(defrule preggente
    ...
    (assert (respuestaprovisional gente (upcase (read))))
)
...
(defrule pregplaya
    ...
    (assert (respuestaprovisional playa (upcase (read))))
)
...
(defrule pregtiempo
    ...
    (assert (RTiempoprovisional (upcase (read))))
)
...
;;;;
;;;;;;;
;;;;;;;;;;;PARTE AÑADIDA UNIDAD DE PRACTICAS 6

;;;;;CASO 1
;Logica por defecto: Si no quiere lugares con mucha gente, le gusta la naturaleza 
(defrule naturaleza_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    (respuesta gente NO)
    (not (respuesta naturaleza NO))
    (not (respuesta naturaleza SI))
    =>
    (assert (respuesta naturaleza SI))
    (printout t "Supongo, por defecto, que le gustará la naturaleza." crlf)
)

;Retractar naturaleza por defecto una vez se ha respondido
(defrule retracta_naturaleza_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (respuestaprovisional naturaleza NO)
    ?d <- (respuesta naturaleza SI)
    =>
    (retract ?p)
    (retract ?d)
    (assert (respuesta naturaleza NO))
    (printout t "Retracto la información supuesta por defecto." crlf)
)

;Confirmar naturaleza
(defrule confirmar_naturaleza_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (respuestaprovisional naturaleza SI)
    (respuesta naturaleza SI)
    =>
    (retract ?p)
)

;Confirmar respuesta naturaleza
(defrule confirmar_respuesta_naturaleza
    (declare (salience 9999))
    (Modulo Preg)
    (not (respuesta naturaleza SI))
    ?p <- (respuestaprovisional naturaleza ?resp)
    =>
    (retract ?p)
    (assert (respuesta naturaleza ?resp))
)



;;;;;CASO 2
;Logica por defecto: Si le gusta la naturaleza, no quiere lugares con mucha gente
(defrule gente_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    (respuesta naturaleza SI)
    (not (respuesta gente SI))
    (not (respuesta gente NO))
    =>
    (assert (respuesta gente NO))
    (printout t "Supongo, por defecto, que no le interesará un lugar con grandes aglomeraciones." crlf)
)

;Retractar gente por defecto una vez se ha respondido
(defrule retracta_gente_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (respuestaprovisional gente SI)
    ?d <- (respuesta gente NO)
    =>
    (retract ?p)
    (retract ?d)
    (assert (respuesta gente SI))
    (printout t "Retracto la información supuesta por defecto." crlf)
)

;Confirmar gente
(defrule confirmar_gente_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (respuestaprovisional gente NO)
    (respuesta gente NO)
    =>
    (retract ?p)
)

;Confirmar respuesta gente
(defrule confirmar_respuesta_gente
    (declare (salience 9999))
    (Modulo Preg)
    (not (respuesta gente NO))
    ?p <- (respuestaprovisional gente ?resp)
    =>
    (retract ?p)
    (assert (respuesta gente ?resp))
)


;;;;;CASO 3
;Logica por defecto: Si quiere calor, le gustará la playa
(defrule playa_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    (RTiempo CALOR)
    (not (respuesta playa NO))
    (not (respuesta playa SI))
    =>
    (assert (respuesta playa SI))
    (printout t "Supongo, por defecto, que le gustará un destino con playa." crlf)
)

;Retractar playa por defecto una vez se ha respondido
(defrule rectractar_playa_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (respuestaprovisional playa NO)
    ?d <- (respuesta playa SI)
    =>
    (retract ?p)
    (retract ?d)
    (assert (respuesta playa NO))
    (printout t "Retracto la información supuesta por defecto." crlf)
)

;Confirmar playa
(defrule confirmar_playa_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (respuestaprovisional playa SI)
    (respuesta playa SI)
    =>
    (retract ?p)
)

;Confirmar respuesta playa
(defrule confirmar_respuesta_playa
    (declare (salience 9999))
    (Modulo Preg)
    (not (respuesta playa SI))
    ?p <- (respuestaprovisional playa ?resp)
    =>
    (retract ?p)
    (assert (respuesta playa ?resp))
)


;;;;;CASO 4
;Logica por defecto: Si le gusta la playa, quiere calor
(defrule calor_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    (respuesta playa SI)
    (not (RTiempo FRIO))
    (not (RTiempo CALOR))
    =>
    (assert (RTiempo CALOR))
    (printout t "Supongo, por defecto, que le gustará el calor." crlf)
)

;Retractar calor por defecto una vez se ha respondido
(defrule rectractar_calor_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    ?p <- (RTiempoprovisional FRIO)
    ?d <- (RTiempo CALOR)
    =>
    (retract ?p)
    (retract ?d)
...
;Confirmar calor
(defrule confirmar_calor_por_defecto
    (declare (salience 9999))
    (Modulo Preg)
    (RTiempo CALOR)
    ?p <- (RTiempoprovisional CALOR)
    =>
    (retract ?p)
)

;Confirmar respuesta calor
(defrule confirmar_respuesta_calor
    (declare (salience 9999))
    (Modulo Preg)
    (not (RTiempo CALOR))
    ?p <- (RTiempoprovisional ?cof)
    =>
    (retract ?p)
    (assert (RTiempo ?cof))
)
