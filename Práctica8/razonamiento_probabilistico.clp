...
;;;; Razonamiento probabilistico: Ejercicio covid  ;;;;;

;Modificar el Ejemplo de integración de razonamiento probabilístico 
;en clips para desarrollar un SE en clips que sea capaz de responder 
;a las tareas del Ejercicio sobre razonamiento con probabilidades.
...
(influye  vacuna Covid)    ; la vacuna influye en la probabilidad de Covid
(efecto test_ Covid)    ; test+ es un efecto o síntoma común de Covid
...
(prob zona_origen bajo_riesgo 0.3)
(prob vacuna si 0.8)
(prob vacuna no 0.2)
)
...
(probcond tos si Covid SI 0.82)
(probcond tos si Covid NO 0.06)
(probcond fiebre si Covid SI 0.8)
(probcond fiebre si Covid NO 0.017)
(probcond test_ si Covid SI 0.95)
(probcond test_ si Covid NO 0.017)
)
...
;preguntar sobre la tasa de covid de la zona de origen
...
;preguntar sobre si tiene la vacuna
(defrule preguntar_vacuna
(red causal causas)
=>
(printout t "Escribe una opcion: Esta vacunado del covid? (1=si 2=no 3=Desconocido): " )
(bind ?respuesta (read))
...
;preguntar sobre si ha tenido fiebre
...
;preguntar sobre si ha tenido tos
(defrule preguntar_tos
(red causal efectos)
=>
(printout t "Ha tenido tos (1=si 2=no 3=Desconocido): " )
(bind ?respuesta (read))
...
;preguntar sobre el resultado del test de covid
(defrule preguntar_test
(red causal efectos)
=>
(printout t "Resultado del test covid (1=positivo 2=negativo 3=Desconocido): " )
(bind ?respuesta (read))
(if (= ?respuesta 1) then (assert (valor test_ si))
  else (if (= ?respuesta 2) then (assert (valor test_ no))
	 else (assert (valor test_ Desconocido))))
(printout t crlf)
)